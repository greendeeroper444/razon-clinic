...
class UserService extends BaseService {
    ...
    async getUsers(queryParams) {
        try {
            ...
            const filter = { role: 'User', isArchived: false };
            ...
            if (limit && parseInt(limit) > 0) {
                ...
                users = await User.find(filter)
                    .select('_id firstName lastName userNumber email contactNumber lastActiveAt createdAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(itemsPerPage)
                    .lean();
    ...
    async getArchivedUsers(queryParams) {
        try {
            const {
                search,
                page,
            ...
            const filter = { role: 'User', isArchived: true };

            if (fromDate || toDate) {
                filter.archivedAt = {};
                if (fromDate) filter.archivedAt.$gte = new Date(fromDate);
                if (toDate) filter.archivedAt.$lte = new Date(toDate);
            }
            ...
            if (limit && parseInt(limit) > 0) {
                ...
                users = await User.find(filter)
                    .select('_id firstName lastName userNumber email contactNumber archivedAt lastActiveAt')
