...
class AppointmentService {
        ...
        if (oldStatus !== appointment.status) {
            ...
            if (oldStatus === 'Pending' && appointment.status === 'Scheduled') {
                try {
                    ...
                    console.error('Failed to auto-create patient from appointment:', error);
                }
            }
        }
    ...
    async createPatientFromAppointment(appointment) {
        try {
            const existingPatient = await PatientService.getPatientsByFilter({
            ...
            if (existingPatient && existingPatient.length > 0) {
                console.log(`Patient already exists for appointment ${appointment.appointmentNumber}`);
                return {
                    created: false,
                    reason: 'patient_exists',
            ...
            const patientData = {
                ...
                suffix: appointment.suffix || undefined,
